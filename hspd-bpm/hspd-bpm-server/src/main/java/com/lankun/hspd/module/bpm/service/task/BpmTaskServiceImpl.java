...
import com.lankun.hspd.framework.common.util.number.NumberUtils;
...
import com.lankun.hspd.framework.datapermission.core.annotation.DataPermission;
import com.lankun.hspd.framework.web.core.util.WebFrameworkUtils;
import com.lankun.hspd.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import com.lankun.hspd.module.bpm.controller.admin.task.vo.task.*;
import com.lankun.hspd.module.bpm.convert.task.BpmTaskConvert;
...
import com.lankun.hspd.module.bpm.enums.definition.*;
import com.lankun.hspd.module.bpm.enums.task.BpmCommentTypeEnum;
import com.lankun.hspd.module.bpm.enums.task.BpmReasonEnum;
import com.lankun.hspd.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import com.lankun.hspd.module.bpm.service.definition.BpmFormService;
import com.lankun.hspd.module.bpm.service.definition.BpmModelService;
...
import com.lankun.hspd.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
//import static com.lankun.hspd.module.bpm.framework.flowable.core.enums.BpmnVariableConstants.*;
import static com.lankun.hspd.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 hspd-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/hspd-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/hspd-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/hspd-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
